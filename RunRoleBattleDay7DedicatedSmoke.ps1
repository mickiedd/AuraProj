...
$Day7ReportPath = Join-Path $ProjectRoot 'Saved\Reports\Day7-Dedicated.json'
$Day7CompositionReportPath = Join-Path $ProjectRoot 'Saved\Reports\Day7-Dedicated-Composition.json'
$Day6ServerLog = Join-Path $ProjectRoot 'Saved\Logs\Day06-Dedicated-Server.log'
...
& $PowerShellExe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $ProjectRoot 'RunRoleBattleDays789Automation.ps1') -Day 7 -Mode Dedicated -EngineRoot $EngineRoot | Out-Host
$Day7ExitCode = [int]$LASTEXITCODE

& $PowerShellExe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $ProjectRoot 'RunRoleBattleDay6NetworkSmoke.ps1') -Mode Dedicated -EngineRoot $EngineRoot | Out-Host
...
Write-Host "[RoleBattleDay7][Dedicated] Day7SpecificStatus=$(if ($Day7SpecificPassed) { 'PASS' } else { 'FAIL' }) (exit $Day7ExitCode)"
Write-Host "[RoleBattleDay7][Dedicated] Day6PrerequisiteStatus=$(if ($Day6PrerequisitePassed) { 'PASS' } else { 'FAIL' }) (exit $Day6ExitCode)"
if (-not $Day7SpecificPassed) { Write-Error "Day 7 dedicated-specific runner failed; see '$Day7ReportPath'." }
if (-not $Day6PrerequisitePassed) { Write-Error "Day 6 dedicated prerequisite failed; see '$Day6ServerLog'." }
