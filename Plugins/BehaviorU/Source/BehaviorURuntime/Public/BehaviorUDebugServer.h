...
/**
 * FBehaviorUDebugServer — 行为树运行时黑板调试 WebSocket 服务器
 *
 * 基于 UE 内置 FSocket (BSD sockets) 实现，零外部模块依赖。
 * 支持 RFC 6455 WebSocket 协议（握手 + text frame）。
 *
 * 控制台命令：
 *   BehaviorU.Debug.StartServer [port]      默认 17654
 *   BehaviorU.Debug.StopServer
 *   BehaviorU.Debug.BroadcastInterval N     0=每帧
 *
 * 协议（UTF-8 JSON）：
 *   Game→Editor: bb_snapshot | agent_removed | agent_tree_source
 *   Editor→Game: request_snapshot
 */
class BEHAVIORURUNTIME_API FBehaviorUDebugServer
{
public:
	FBehaviorUDebugServer();
	~FBehaviorUDebugServer();

	FBehaviorUDebugServer(const FBehaviorUDebugServer&)            = delete;
	FBehaviorUDebugServer& operator=(const FBehaviorUDebugServer&) = delete;
	...
	void BroadcastSnapshots(const TArray<UBehaviorUAgentComponent*>& Agents);
	 ...
	 */
	void BroadcastTreeSourcePaths(const TArray<UBehaviorUAgentComponent*>& Agents);
...
private:
	...
	static FString BuildSnapshotJson(const FBehaviorUBlackboardSnapshot& Snap);
