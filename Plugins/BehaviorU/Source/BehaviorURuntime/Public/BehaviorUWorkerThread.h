...
/**
 * FBehaviorUWorkerThread — 持久化行为树工作线程
 *
 * 线程池中的单个工作单元。每帧由 UBehaviorUWorldSubsystem 分配一批连续的 Agent
 ...
 */
class BEHAVIORURUNTIME_API FBehaviorUWorkerThread : public FRunnable
{
public:
	/**
	 * @param InThreadIndex 线程序号，用于命名（0..N-1）
	 */
	explicit FBehaviorUWorkerThread(int32 InThreadIndex);
	virtual ~FBehaviorUWorkerThread();
	 ...
	 * 必须在 StartWork() 之前调用。
	 */
	void AssignWork(UBehaviorUAgentComponent** InAgents, int32 InCount);
...
private:
	 ...
	 */
	UBehaviorUAgentComponent** Agents = nullptr;
