...
#include "BehaviorUWorldSubsystem.generated.h"

class UBehaviorUAgentComponent;

/**
 * UBehaviorUWorldSubsystem — 行为树并行 Tick 子系统
 *
 ...
 */
UCLASS()
class BEHAVIORURUNTIME_API UBehaviorUWorldSubsystem : public UTickableWorldSubsystem
{
	GENERATED_BODY()
...
public:
	...
	/**
	 * 注册一个 Agent，由 UBehaviorUAgentComponent::BeginPlay 自动调用。
	 * 注册后设置 bManagedBySubsystem=true 并禁用 ComponentTick。
	 */
	void RegisterAgent(UBehaviorUAgentComponent* Agent);

	/**
	 * 注销一个 Agent，由 UBehaviorUAgentComponent::EndPlay 自动调用。
	 */
	void UnregisterAgent(UBehaviorUAgentComponent* Agent);

	/** 当前注册的 Agent 数量（调试/性能统计） */
	UFUNCTION(BlueprintPure, Category = "BehaviorU|Subsystem")
	...
	UFUNCTION(BlueprintPure, Category = "BehaviorU|Debug")
...
private:
	...
	TArray<TWeakObjectPtr<UBehaviorUAgentComponent>> RegisteredAgents;

	/**
	 * 行为树 Tick 频率限制累积器。
	 * 每帧累加 DeltaTime，超过目标间隔才执行一次行为树 Tick，
	 * 与 BehaviorU.TickRate 控制台变量配合使用。
	 */
	...
	TArray<TUniquePtr<FBehaviorUWorkerThread>> WorkerThreads;
	 ...
	 */
	void Phase1_ProcessCommandQueues(TArray<UBehaviorUAgentComponent*>& OutActiveAgents);

	/**
	 * Phase 2：将 Agent 切片分发给工作线程池并等待全部完成。
	 */
	void Phase2_TickBehaviorTrees(TArray<UBehaviorUAgentComponent*>& ActiveAgents);
};
