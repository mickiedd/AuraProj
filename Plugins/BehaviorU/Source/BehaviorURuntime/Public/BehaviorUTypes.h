...
/**
 * Console variable that controls verbose BehaviorU debug logging.
 * Enable at runtime:  BehaviorU.VerboseLogging 1
 * Or in DefaultEngine.ini:
 *   [SystemSettings]
 *   BehaviorU.VerboseLogging=1
 */
BEHAVIORURUNTIME_API extern TAutoConsoleVariable<int32> CVarBehaviorUVerboseLogging;

/**
 * Verbose log macro — only emits when BehaviorU.VerboseLogging is non-zero.
 * Use this instead of UE_LOG(LogTemp, Warning, ...) for noisy per-frame / init spam.
 * Real errors should still use UE_LOG(LogBehaviorU, Error, ...) directly.
 *
 * 日志前缀：[GT] = 游戏线程，[WT:id] = 工作线程（id 为系统线程 ID）
 * 可安全在任意线程调用（使用 GetValueOnAnyThread）。
 */
FORCEINLINE FString BehaviorU_GetThreadTag()
{
	if (IsInGameThread())
	{
		return TEXT("[GT]");
...
#define BEHAVIORU_VLOG(Format, ...) \
	if (CVarBehaviorUVerboseLogging.GetValueOnAnyThread() != 0) \
	{ \
		UE_LOG(LogBehaviorU, Log, TEXT("%s ") Format, *BehaviorU_GetThreadTag(), ##__VA_ARGS__); \
	}
...
#define BEHAVIORU_INVALID_NODE_ID (-2)
 ...
 */
FORCEINLINE bool BehaviorUIsVectorString(const FString& Str)
{
 ...
 */
FORCEINLINE FString BehaviorUVectorToString(const FVector& V)
{
	return V.ToString();
}
 ...
 */
FORCEINLINE FVector BehaviorUStringToVector(const FString& Str)
{
