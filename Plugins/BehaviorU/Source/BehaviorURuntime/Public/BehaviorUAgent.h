 ...
 */
struct FBehaviorUNodeSnapshot
{
	/** 节点 ID（来自 UBehaviorUBehaviorNode::NodeId，< 0 表示无效包装节点） */
	int32   NodeId    = -2;
 ...
 */
struct FBehaviorUBlackboardSnapshot
{
	...
	TArray<FBehaviorUNodeSnapshot> ActiveNodes;
};
 ...
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FBehaviorUMethodNameDelegate, const FString&, MethodName);
...
public:
	 ...
	 */
	UPROPERTY(BlueprintAssignable, Category = "BehaviorU|Methods")
	FBehaviorUMethodNameDelegate OnMethodNameCalled;
	 ...
	 * @param Result      The EBehaviorUStatus value to return from ExecuteMethod.
	 */
	UFUNCTION(BlueprintCallable, Category = "BehaviorU|Methods")
	void SetTSMethodResult(const FString& MethodName, EBehaviorUStatus Result);
...
protected:
	 ...
	 */
	TMap<FString, EBehaviorUStatus> MethodNameResults;
	...
	/**
	 * 是否已向 UBehaviorUWorldSubsystem 注册。
	 ...
	 */
	TArray<FBehaviorUCommand> PendingCommandQueue;

	/**
	 * 命令执行结果表：CommandId → EBehaviorUStatus。
	 * ProcessCommandQueue 写入，GetCommandResult 消费（读取即删除）。
	 */
	TMap<uint64, EBehaviorUStatus> CommandResults;
...
public:
	 ...
	 */
	bool GetCommandResult(uint64 CommandId, EBehaviorUStatus& OutStatus);
	 ...
	 * 由 UBehaviorUWorldSubsystem 在 Phase 1 统一调用；
	 ...
	 */
	FBehaviorUBlackboardSnapshot GetBlackboardSnapshot() const;

	/** 获取当前加载的行为树名称（来自 UBehaviorUBehaviorTree::TreeName），未加载时返回空字符串 */
	...
	/**
	 * 由 UBehaviorUWorldSubsystem 在注册/注销时调用，标记当前管理状态。
