...
// BehaviorUTypes.h 已包含 CoreMinimal.h，且 clangd 已为其建立索引
 ...
 * 后续扩展时在此添加新类型。
 */
enum class EBehaviorUCommandType : uint8
{
 ...
 * 行为树命令消息。
 *
 * 由 Action 节点在树 Tick 期间生产，压入 UBehaviorUAgentComponent 的命令队列。
 ...
 */
struct FBehaviorUCommand
{
	/** 命令类型 */
	EBehaviorUCommandType Type = EBehaviorUCommandType::ExecuteMethod;
