...
FBehaviacDebugServer::FBehaviacDebugServer()  = default;

FBehaviacDebugServer::~FBehaviacDebugServer()
{
	Stop();
}
...
bool FBehaviacDebugServer::Start(uint32 Port)
{
	if (bRunning)
	{
		UE_LOG(LogBehaviac, Warning, TEXT("[BehaviacDebug] 服务器已在运行（端口 %u）。"), ListenPort);
		return false;
	}
	...
	if (!SocketSS)
	{
		UE_LOG(LogBehaviac, Error, TEXT("[BehaviacDebug] 无法获取 SocketSubsystem。"));
		return false;
	}

	// 创建 TCP 监听套接字
	ListenSocket = SocketSS->CreateSocket(NAME_Stream, TEXT("BehaviacDebugListen"), false);
	if (!ListenSocket)
	{
		UE_LOG(LogBehaviac, Error, TEXT("[BehaviacDebug] CreateSocket 失败。"));
		return false;
	}
	...
	{
		UE_LOG(LogBehaviac, Error, TEXT("[BehaviacDebug] Bind 失败（端口 %u 可能已占用）。"), Port);
	...
	{
		UE_LOG(LogBehaviac, Error, TEXT("[BehaviacDebug] Listen 失败。"));
		SocketSS->DestroySocket(ListenSocket);
		ListenSocket = nullptr;
		return false;
	}
	...
	UE_LOG(LogBehaviac, Log, TEXT("[BehaviacDebug] WebSocket 调试服务器已启动，监听端口 %u"), Port);
	return true;
}

void FBehaviacDebugServer::Stop()
{
	if (!bRunning)
	{
		return;
	...
	UE_LOG(LogBehaviac, Log, TEXT("[BehaviacDebug] WebSocket 调试服务器已停止。"));
}
...
void FBehaviacDebugServer::Tick()
{
	if (!bRunning) { return; }
...
void FBehaviacDebugServer::BroadcastSnapshots(const TArray<UBehaviacAgentComponent*>& Agents)
{
	...
	{
		FBehaviacBlackboardSnapshot Snapshot;
		int32 SwitchScore = 0;
		int32 OriginalIndex = 0;
	};

	auto CalculateNodeSwitchScore = [](const FBehaviacBlackboardSnapshot& Snapshot,
		...
		FAgentActivityState& OutNextState) -> int32
	{
		...
		for (const FBehaviacNodeSnapshot& Node : Snapshot.ActiveNodes)
		{
	...
	{
		const UBehaviacAgentComponent* Agent = Agents[AgentIndex];
		if (!Agent) { continue; }
...
void FBehaviacDebugServer::SendAgentRemoved(uint64 AgentPtrId)
{
...
void FBehaviacDebugServer::AcceptPending()
{
	...
	while (ListenSocket->HasPendingConnection(bHasPending) && bHasPending)
	{
		...
		FSocket* NewSock = ListenSocket->Accept(*RemoteAddr, TEXT("BehaviacDebugClient"));
		if (!NewSock) { break; }
		...
		UE_LOG(LogBehaviac, Log, TEXT("[BehaviacDebug] 客户端已连入，当前连接数: %d"), Clients.Num());
	}
}
...
void FBehaviacDebugServer::ReceiveFrom(FClientConn* Conn)
{
...
void FBehaviacDebugServer::HandleHandshake(FClientConn* Conn)
{
	// 等到收到完整的 HTTP 请求头（以 \r\n\r\n 结尾）
	...
	{
		UE_LOG(LogBehaviac, Warning, TEXT("[BehaviacDebug] 握手请求缺少 Sec-WebSocket-Key，拒绝连接。"));
		ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->DestroySocket(Conn->Socket);
		Conn->Socket = nullptr;
		return;
	}
	...
	UE_LOG(LogBehaviac, Log, TEXT("[BehaviacDebug] WebSocket 握手完成，客户端就绪。"));
...
void FBehaviacDebugServer::HandleFrames(FClientConn* Conn)
{
	...
	while (Buf.Num() >= 2)
	{
		...
		if (Opcode == 1 && Payload.Num() > 0)
		{
			...
			{
				UE_LOG(LogBehaviac, Verbose, TEXT("[BehaviacDebug] 收到 request_snapshot。"));
				bImmediateSnapshotRequested = true;
			}
		}
	}
...
void FBehaviacDebugServer::RemoveClient(int32 Index)
{
	...
	UE_LOG(LogBehaviac, Log, TEXT("[BehaviacDebug] 客户端已断开，剩余连接数: %d"), Clients.Num());
}
...
void FBehaviacDebugServer::SendTextFrame(FClientConn* Conn, const FString& JsonStr)
{
...
void FBehaviacDebugServer::BroadcastJson(const FString& JsonStr)
{
	for (int32 i = Clients.Num() - 1; i >= 0; --i)
	{
...
/* static */
FString FBehaviacDebugServer::JsonEscape(const FString& Str)
{
	FString Out;
...
/* static */
FString FBehaviacDebugServer::BuildSnapshotJson(const FBehaviacBlackboardSnapshot& Snap)
{
	FString Json;
	...
	Json += TEXT("},\"nodes\":[");
	bFirst = true;
	for (const FBehaviacNodeSnapshot& N : Snap.ActiveNodes)
	{
		if (!bFirst) { Json += TEXT(","); }
		Json += TEXT("{\"id\":");
...
/* static */
FString FBehaviacDebugServer::BuildRemovedJson(uint64 AgentPtrId)
{
	return FString::Printf(TEXT("{\"type\":\"agent_removed\",\"agent_id\":\"%llu\"}"), AgentPtrId);
}

/* static */
FString FBehaviacDebugServer::BuildTreeSourcePathJson(uint64 AgentPtrId, const FString& AgentName, const FString& TreeName, const FString& SourcePath)
{
	FString Json;
...
void FBehaviacDebugServer::BroadcastTreeSourcePaths(const TArray<UBehaviacAgentComponent*>& Agents)
{
	if (!bRunning || Clients.IsEmpty()) { return; }
	...
	UE_LOG(LogBehaviac, Log, TEXT("[BehaviacDebug][TreeSync] 检测到未同步客户端，开始收集 Agent 行为树 XML 路径。当前 Agent 数: %d"), Agents.Num());
	...
	for (const UBehaviacAgentComponent* Agent : Agents)
	{
		if (!Agent) { continue; }
		...
		UE_LOG(LogBehaviac, Log,
			TEXT("[BehaviacDebug][TreeSync]   Agent=%s  TreeName='%s'  SourcePath='%s'"),
	...
	if (TreeSourcePaths.IsEmpty())
	{
		UE_LOG(LogBehaviac, Warning,
			TEXT("[BehaviacDebug][TreeSync] 未收集到任何可用行为树 XML 路径（Agent 可能尚未加载树，或树未保留源文件路径）。"));
		return;
	}

	UE_LOG(LogBehaviac, Log, TEXT("[BehaviacDebug][TreeSync] 收集到 %d 个 Agent 的行为树路径，准备推送。"), TreeSourcePaths.Num());
	...
	for (FClientConn* Conn : Clients)
	{
		...
		for (const FTreeSourcePathPayload& Payload : TreeSourcePaths)
		{
			...
			UE_LOG(LogBehaviac, Log,
				TEXT("[BehaviacDebug][TreeSync] 推送 Agent=%s Tree='%s' Path='%s'，JSON 总字节数: %d"),
				*Payload.AgentName, *Payload.TreeName, *Payload.SourcePath, Json.Len());
			SendTextFrame(Conn, Json);
		}

		Conn->bTreePathsSynced = true;
		UE_LOG(LogBehaviac, Log, TEXT("[BehaviacDebug][TreeSync] 已向客户端推送 %d 个 Agent 的行为树路径，标记同步完成。"), TreeSourcePaths.Num());
	}
}
