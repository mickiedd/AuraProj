...
FBehaviacWorkerThread::FBehaviacWorkerThread(int32 InThreadIndex)
	: bShouldStop(false)
{
	...
	const FString ThreadName = FString::Printf(TEXT("BehaviacWorker_%d"), InThreadIndex);
	Thread = FRunnableThread::Create(this, *ThreadName, 0, TPri_BelowNormal);

	BEHAVIAC_VLOG(TEXT("[Behaviac] 工作线程 %s 已创建"), *ThreadName);
}

FBehaviacWorkerThread::~FBehaviacWorkerThread()
{
	Shutdown();
...
bool FBehaviacWorkerThread::Init()
{
	return true;
}

uint32 FBehaviacWorkerThread::Run()
{
	while (true)
	{
		...
		{
			SCOPE_CYCLE_COUNTER(STAT_Behaviac_WorkerTickAgents);
			for (int32 i = 0; i < AgentCount; ++i)
			{
				UBehaviacAgentComponent* Agent = Agents[i];
...
void FBehaviacWorkerThread::Stop()
{
	bShouldStop = true;
...
void FBehaviacWorkerThread::AssignWork(UBehaviacAgentComponent** InAgents, int32 InCount)
{
...
void FBehaviacWorkerThread::StartWork()
{
	WorkReadyEvent->Trigger();
}

void FBehaviacWorkerThread::WaitDone()
{
	WorkDoneEvent->Wait();
}
...
void FBehaviacWorkerThread::Shutdown()
{
	if (Thread)
	{
