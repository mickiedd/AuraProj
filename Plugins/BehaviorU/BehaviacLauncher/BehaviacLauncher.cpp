// BehaviacLauncher.cpp
//
// Standalone Windows launcher for the BehaviorU Behavior Tree Editor.
//   1. Starts  Plugins/BehaviacPlugin/Editor/behaviac_server.py  on port 18102.
...
//
// Build:  run build.bat
// Output: Tools/BehaviacLauncher/BehaviacLauncher.exe
...
static const WCHAR kWindowClass[] = L"BehaviacLauncherWnd";
...
{
    const std::wstring script = g_editorDir + L"\\behaviac_server.py";
...
static bool StartPythonServer()
{
    ...
    if (!PathExists(script))
    {
        ...
        MessageBoxW(g_hwnd, msg.c_str(), L"BehaviacLauncher", MB_ICONERROR);
        return false;
    }
...
static void OpenBrowserExternal()
{
    ...
    if (reinterpret_cast<INT_PTR>(result) <= 32)
    {
        MessageBoxW(g_hwnd,
            ...
            L"BehaviacLauncher", MB_ICONWARNING);
    }
}
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, LPWSTR, int)
{
    ...
    HANDLE hMutex = CreateMutexW(nullptr, TRUE, L"BehaviacLauncher_SingleInstance");
    if (hMutex == nullptr || GetLastError() == ERROR_ALREADY_EXISTS)
    {
        MessageBoxW(nullptr,
            L"BehaviacLauncher is already running.\n\nOnly one instance may run at a time.",
            L"BehaviacLauncher", MB_ICONWARNING | MB_OK);
    ...
    g_serverLogPath = exeDir + L"\\behaviac_server.log";
    ...
    if (!PathExists(g_editorDir))
    {
        const std::wstring msg =
            ...
            L"\n\nExpected layout:\n"
            L"  <ProjectRoot>\\Plugins\\BehaviacPlugin\\BehaviacLauncher\\BehaviacLauncher.exe\n"
            L"  <ProjectRoot>\\Plugins\\BehaviacPlugin\\Editor\\";
        MessageBoxW(nullptr, msg.c_str(), L"BehaviacLauncher", MB_ICONERROR);
        return 1;
    }
    ...
    if (!g_hwnd)
    {
        MessageBoxW(nullptr, L"Failed to create application window.", L"BehaviacLauncher", MB_ICONERROR);
        return 1;
    }
    ...
    else
    {
        if (!StartPythonServer())
        {
            MessageBoxW(g_hwnd,
                ...
                L"BehaviacLauncher", MB_ICONERROR);
