...
{
    SharedCostGE = UGameplayEffect::StaticClass();
...
void UAuraDataAbility::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
    ...
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] ActivateAbility ApplyCost done"));
    ApplyCooldown(Handle, ActorInfo, ActivationInfo);
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] ActivateAbility ApplyCooldown done"));

    if (const UAuraAbilityDefinition* Definition = GetDefinition())
    {
        UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] Activating %s (RootNode=%s)"), *Definition->AbilityTag.ToString(), Definition->RootNode ? *Definition->RootNode->NodeClassName : TEXT("null"));
    }
    else
    {
    ...
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] RootTask initial Execute returned %s"), *StaticEnum<EAuraAbilityActionStatus>()->GetValueAsString(Status));
...
void UAuraDataAbility::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
    ...
    if (RootTask)
    {
        UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] EndAbility cancelling RootTask"));
    ...
    {
        UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] EndAbility resetting PendingTargetDataTask"));
        PendingTargetDataTask.Reset();
    }
    if (PendingMontageTask.IsValid())
    {
        UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] EndAbility resetting PendingMontageTask"));
        PendingMontageTask.Reset();
    }
    if (PendingMontageEventTask.IsValid())
    {
        UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] EndAbility resetting PendingMontageEventTask"));
    ...
    PersistentCtx.TargetDataHandle.Clear();
    PersistentCtx.CursorHit = FHitResult();
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] EndAbility END"));
...
{
    if (!GetDefinition() || !ActorInfo || !ActorInfo->AbilitySystemComponent.IsValid())
    {
        return;
    }
    ...
    if (const UAuraAttributeSet* AttributeSet = Cast<UAuraAttributeSet>(ActorInfo->AbilitySystemComponent->GetAttributeSet(UAuraAttributeSet::StaticClass())))
    {
        float CurrentMana = AttributeSet->GetMana();
        float NewMana = FMath::Max(0.f, CurrentMana - GetDefinition()->ManaCost);
        const_cast<UAuraAttributeSet*>(AttributeSet)->SetMana(NewMana);
        UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] ApplyCost mana=%.1f->%.1f cost=%.1f"), CurrentMana, NewMana, GetDefinition()->ManaCost);
    }
}
...
{
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] AdvanceGraph called with ChildStatus=%s"), *StaticEnum<EAuraAbilityActionStatus>()->GetValueAsString(ChildStatus));
    if (!bGraphActive)
    {
    ...
    {
        EAuraAbilityActionStatus Status = RootTask->Execute(PersistentCtx);
        UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] AdvanceGraph RootTask->Execute returned %s"), *StaticEnum<EAuraAbilityActionStatus>()->GetValueAsString(Status));
        if (Status != EAuraAbilityActionStatus::Running)
        {
            UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] AdvanceGraph ending ability because status=%s"), *StaticEnum<EAuraAbilityActionStatus>()->GetValueAsString(Status));
...
{
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] OnTargetDataReady DataNum=%d bGraphActive=%s"), DataHandle.Num(), bGraphActive ? TEXT("true") : TEXT("false"));
    if (!bGraphActive)
    {
    ...
    if (PersistentCtx.AvatarActor && PersistentCtx.CursorHit.bBlockingHit)
    {
        ...
        if (!Direction.IsZero())
        {
            ...
            UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] OnTargetDataReady rotated character to %s"), *TargetRotation.ToString());
        }
    }
...
{
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] OnMontageEventReceived bGraphActive=%s"), bGraphActive ? TEXT("true") : TEXT("false"));
    if (!bGraphActive)
    {
        return;
    }
    ...
    if (PendingMontageEventTask.IsValid())
    {
        UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] OnMontageEventReceived resetting PendingMontageEventTask"));
        PendingMontageEventTask.Reset();
    }
...
{
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] OnMontageCompleted bGraphActive=%s"), bGraphActive ? TEXT("true") : TEXT("false"));
    if (!bGraphActive)
    {
        return;
    }
    ...
    if (PendingMontageTask.IsValid())
    {
        UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] OnMontageCompleted resetting PendingMontageTask"));
        PendingMontageTask.Reset();
    }
...
{
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[DataAbility] OnMontageInterrupted bGraphActive=%s"), bGraphActive ? TEXT("true") : TEXT("false"));
    if (!bGraphActive)
    {
        return;
    }
