...
{
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[MulticastGunFX] OnStart OwnerAbility=%s Avatar=%s"), *GetNameSafe(OwnerAbility), *GetNameSafe(Ctx.AvatarActor));
    ...
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[MulticastGunFX] OnStart MuzzleSocketTag=%s MuzzleLoc=%s"), *Node->MuzzleSocketTag.ToString(), *MuzzleLocation.ToString());

    TObjectPtr<UParticleSystem> MuzzleEffect = LoadObject<UParticleSystem>(nullptr, *Node->MuzzleEffect);
    TObjectPtr<USoundBase> FireSound = LoadObject<USoundBase>(nullptr, *Node->FireSound);
    UE_LOG(LogAuraAbilityGraph, Log, TEXT("[MulticastGunFX] OnStart Effect=%s Sound=%s"), *GetNameSafe(MuzzleEffect), *GetNameSafe(FireSound));
    ...
    if (AAuraCharacterBase* AuraCharacter = Cast<AAuraCharacterBase>(Ctx.AvatarActor))
    {
        ...
        UE_LOG(LogAuraAbilityGraph, Log, TEXT("[MulticastGunFX] OnStart multicast FX sent"));
    }
    else
    {
