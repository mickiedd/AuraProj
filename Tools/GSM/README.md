...
The top-level configuration may set `requestDeadlineSeconds` (default 30) and `serverStartupDeadlineSeconds` (default 900). Both must be finite values from 0.1 through 3600 seconds, and the server startup deadline must be greater than the request deadline. The status API exposes the effective values.
...
- Each client request has a 30-second response deadline. Concurrent requests coalesce onto one launch. If a request expires while the child is still starting, GSM returns an explicit retryable error (`retryable: true`) but preserves the progressing child and its nonce. The Login flow automatically retries that response with a one-second backoff while the Loading level remains visible; other callers can retry at their discretion. A later request reuses that process and returns immediately once authenticated world readiness arrives.
- Each level has a separate 15-minute server startup deadline. Only that deadline closes the ownership job and fails the level. Process exits still invalidate readiness and preserve the native exit code. GSM does not automatically restart failed processes.
