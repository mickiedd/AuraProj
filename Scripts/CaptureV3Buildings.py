"""Light isolated V3 preview maps and capture the actual Blueprint instances.
...
def capture(index, view='front'):
    ...
    if not any(a.get_actor_label() == 'V3_Preview_Key' for a in all_actors):
        ...
        fill.set_actor_label('V3_Preview_Fill')
    ...
    all_actors = actors.get_all_level_actors()
    for label, pitch, yaw in [('V3_Preview_Key', -40, -60), ('V3_Preview_Fill', -30, 120)]:
    ...
    if not any(a.get_actor_label() == 'V3_Preview_Ground' for a in all_actors):
        floor = actors.spawn_actor_from_class(unreal.StaticMeshActor, unreal.Vector(0, 0, -20))
        floor.set_actor_label('V3_Preview_Ground')
    ...
    floor = next(a for a in actors.get_all_level_actors() if a.get_actor_label() == 'V3_Preview_Ground')
    ground_path = '/Game/Assets/Environment/GuangzhouLandmarks/V3/M_V3PreviewGround'
    ...
    if material is None:
        material = unreal.AssetToolsHelpers.get_asset_tools().create_asset('M_V3PreviewGround',
            '/Game/Assets/Environment/GuangzhouLandmarks/V3', unreal.Material, unreal.MaterialFactoryNew())
    ...
    camera = target + unreal.Vector(radius * 1.2 * sign, radius * 2.15 * sign, radius * .85)
    ...
    print('V3_CAPTURE_REQUESTED', str(output))
