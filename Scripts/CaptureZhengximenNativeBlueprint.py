"""Viewport-independent visual capture of BP_Zhengximen (SceneCapture2D).

WHY
---
Structural validation - the asset contract, the Nanite fallback, the bounds - says
nothing about whether the gate *looks* like the reference. "GUI viewport review"
tends to stay pending forever, and a human screenshot is not reproducible, so this
renders the actual native Blueprint assets into PNGs from a script.

WHAT IT DOES
------------
Spawns BP_Zhengximen far from the level, derives cameras from its bounds, lights
the scene with temporary actors, renders through a transient SceneCapture2D into a
transient render target, writes PNGs, then destroys every temporary actor and
asserts none leaked. It never saves a map.

The gate's facade - the plaque and the iron door - is on local -Y, so the front
camera is placed on the -Y side.

THIS NEEDS AN OPEN EDITOR WITH A LIVE RHI.
A Python commandlet has neither an editor world nor rendering, so it cannot run
there; it will silently produce no image. Run it in the running editor:

    UE_ENGINE_ROOT=/Volumes/M2/Engine/UE_5.5 \\
      python3 Scripts/remote_run.py Scripts/CaptureZhengximenNativeBlueprint.py

or from the editor's Python console:

    py "/Volumes/M2/Works/AuraProj/Scripts/CaptureZhengximenNativeBlueprint.py"

GOTCHAS BAKED IN
----------------
* `RenderingLibrary.read_render_target(world, rt, True)` returns rows top-down on
  macOS/Metal builds, which the PNG writer assumes.
* Exposure: manual exposure with a high `auto_exposure_bias` is needed or the
  capture comes back nearly black.
* `capture_scene()` is called twice so the render target is populated before read.
* Transient actors only; assert `before == after` to catch leaks, and never save.
"""
...
OUT_DIR = PROJECT / "Saved" / "Reports" / "Zhengximen"
WIDTH, HEIGHT = 1600, 1000

# (name, camera offset as a multiple of the building's extents)
VIEWS = [
    ("front", (0.0, -1.75, 0.42)),
    ("three-quarter", (0.95, -1.55, 0.55)),
]
...
def main():
    ...
    try:
        ...
        for name, offset in VIEWS:
            camera = target + unreal.Vector(extent.x * offset[0],
                                            extent.x * offset[1],
                                            extent.z * offset[2])
            ...
            settings.set_editor_property("auto_exposure_bias", 12.0)
            ...
            pixels = unreal.RenderingLibrary.read_render_target(world, render_target,
                                                                True)
            assert len(pixels) == WIDTH * HEIGHT, len(pixels)
            out = OUT_DIR / ("native-{}.png".format(name))
    ...
    print("ZHENGXIMEN_CAPTURE_COMPLETE " + json.dumps({"views": results}))
