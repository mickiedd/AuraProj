"""Remove BP_Xiaobeimen_Production_V3's actor from Scifi_desert_city/Level/L_showcase_level.

The ring showcase is not the only level holding this Blueprint. L_showcase_level
carries one actor spawned from it - GuangzhouLandmark_Xiaobeimen_Production_V3 -
and deleting the Blueprint while that actor still exists would leave the level
with a dangling reference, so the actor goes first and the level is saved.
...
different assets and are left alone. Anything attached to the doomed actor is
reported before it is destroyed, since destroying a parent takes its children
with it.
