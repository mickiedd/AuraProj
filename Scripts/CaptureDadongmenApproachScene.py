"""Render the rear approach side to verify the two replacement ground pieces."""
import json, time
...
capture_actor = actors.spawn_actor_from_class(unreal.SceneCapture2D, unreal.Vector(0, -4600, 1250))
capture_actor.set_actor_label('V4_Dadongmen_ApproachSceneCapture')
target = unreal.Vector(0, -500, 450)
...
time.sleep(3)
unreal.RenderingLibrary.export_render_target(world, render_target, str(ROOT), 'Dadongmen_V4-approach-scene')
time.sleep(2)
actors.destroy_actor(capture_actor)
unreal.EditorLoadingAndSavingUtils.save_map(world, report['preview_level'])
print('DADONGMEN_APPROACH_SCENE_READY', str(ROOT / 'Dadongmen_V4-approach-scene'))
