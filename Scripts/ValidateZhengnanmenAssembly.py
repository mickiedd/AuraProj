...
wall_report=OUT.parent/'ZhengnanmenFlatMasonry/apply.json'
intact_report=OUT.parent/'ZhengnanmenIntactPerspective/apply.json'
expected=json.loads((intact_report if intact_report.exists() else wall_report if wall_report.exists() else OUT/'apply.json').read_text())['after']
expected_instances=sum(r['instances'] for r in expected.values())
actual=snapshot(templates(bp)[1])
assert actual==expected,'Serialized component data differs from repaired snapshot'
assert len(actual)==114 and sum(r['instances'] for r in actual.values())==expected_instances
...
if mode=='live':
    ...
    try:
        ...
        assert sum(c.get_instance_count() for c in cs)==expected_instances
    finally:assert sub.destroy_actor(a)
report=dict(passed=True,mode=mode,blueprint=BP,components=114,instances=expected_instances,roof_shells=3,signboard_faces=1,
