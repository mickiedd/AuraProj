...
$WorldPersistenceId = "RoleBattleDay4-$Mode-$RunId"

if ([string]::IsNullOrWhiteSpace($Mode)) {
    Write-Error 'Specify -Mode Listen or Dedicated explicitly.'
    exit 1
}
...
try {
    ...
    $ServerArguments = @(
        ...
        "-port=$ListenPort", '-RoleBattleDay4DamageProbe', "-WorldPersistenceId=$WorldPersistenceId", '-AuraPersistenceProvider=NULL', "-abslog=$ServerLog"
    ...
    do {
        ...
        $Assertions = [ordered]@{
            Client1InvalidDamageRequestSent = Test-Pattern $Client1Log '\[Day4DamageProbe\]\[Client\] InvalidDamageRequestSent=1'
            ServerReceivedClientRequest = Test-Pattern $ServerLog '\[Day4DamageProbe\]\[Server\] ClientInvalidDamageRequestReceived=1'
            ServerRejectedInvalidDamage = Test-Pattern $ServerLog '\[Day4DamageProbe\]\[Server\].*InvalidDamageRejected=1'
            ServerHealthUnchanged = Test-Pattern $ServerLog '\[Day4DamageProbe\]\[Server\].*HealthChanged=0'
        }
...
if (-not $Passed) {
    Write-Error "[Day4DamageSmoke][$Mode] FAIL: $Failure"
    exit 1
}
Write-Host "[Day4DamageSmoke][$Mode] PASS"
exit 0
