...
Aura uses AuraWebUI for the Login, Loading, and complete gameplay HUD presentations while keeping gameplay authority and GAS state in native C++. The implementation is isolated in the `Plugins/AuraWebUI` runtime plugin.
...
- `Plugins/AuraWebUI/Content/WebUI/hud-left-top.html`, `hud-right-top.html`, `hud-bottom.html`, and `hud-interaction.html` are the four transparent gameplay HUD panels. Their native `UWebUIWidget` hosts are bounded to the left-top, right-top, bottom, and centered interaction regions so the uncovered level viewport remains native-hit-testable.
...
The Loading level is migrated. `ALoadingPlayerController` still owns all travel, portal resolution, timeout, and progress timing; only its presentation is WebUI via `WebUI/loading.html`. Progress is sent as the `loading_progress` event, with `{ "percent": 0-100, "message": "..." }` payload data. The page sends `ready` when its browser socket opens and updates its progress bar from the event.
...
The gameplay HUD is also migrated. `AAuraHUD` creates four native `UWebUIWidget` hosts and nonvisual widget controllers, then loads the four panel pages into bounded viewport slots. The bridge carries vitals, progress, equipped abilities, the complete spell catalog, attribute rows, interaction previews, messages, location, and quit modal state to all connected panels. WebUI commands call validated controller/player-controller methods directly; the legacy WBP overlay is not constructed and no Canvas/native HUD fallback is restored on browser disconnect. The ordinary level viewport is not covered by a WebUI browser and no panel synthesizes world LMB events from blank-page pointer input; only an explicit skill button sends ability press/hold/release commands. Attribute, spell, and quit panels expand their owning browser surface while active. The interaction preview has its own centered browser surface, is collapsed when no target descriptor is focused, and never resizes the bottom skill/vitals panel. Skill icons use the bottom WebUI panel with modern framed and ability-type-colored treatments.
