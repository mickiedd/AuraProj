...
- `Plugins/AuraWebUI/Content/WebUI/hud.html` is the full-screen transparent gameplay HUD: vitals, progress, skill slots, attributes, spells, interaction previews, messages, location, and quit confirmation.
...
The gameplay HUD is also migrated. `AAuraHUD` creates only the native `UWebUIWidget` host and nonvisual widget controllers, then loads `WebUI/hud.html` across the full transparent viewport. The bridge carries vitals, progress, equipped abilities, the complete spell catalog, attribute rows, interaction previews, messages, location, and quit modal state. WebUI commands call validated controller/player-controller methods directly; the legacy WBP overlay is not constructed and no Canvas/native HUD fallback is restored on browser disconnect. Because the UE browser viewport captures pointer input, `hud.html` forwards world LMB press/hold/release to the player controller and releases it on pointer cancellation, window blur, or browser disconnect; HUD buttons are excluded from that gameplay route. Skill icons use the same WebUI page with modern framed and ability-type-colored treatments.
