...
- `Plugins/AuraWebUI/Content/WebUI/hud-left-top.html`, `hud-right-top.html`, and `hud-bottom.html` are the three transparent gameplay HUD panels. Their native `UWebUIWidget` hosts are bounded to the left-top, right-top, and bottom regions so the uncovered level viewport remains native-hit-testable.
