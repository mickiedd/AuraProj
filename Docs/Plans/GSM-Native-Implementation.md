...
5. Enforce a 30-second per-request response deadline separately from the 15-minute server startup deadline. Concurrent waiters share one launch; an expired request receives an explicit `retryable: true` response without terminating a healthy, progressing child. The Login-to-Loading client flow retries that response automatically with a one-second backoff while retaining the pending travel. Only process exit, the server startup deadline, or manager shutdown closes the owned job. Record PID, exit code, launch count and failure; never kill processes by name or port.
