...
- **Single entry point:** new checked-in root `RunPlayableCandidate.ps1` with `-Stage Fast|Candidate|External`, `-Soak`, `-Finalize`, `-Revision`, `-Mode Listen|Dedicated|Both`, `-Role Aura|BungeeMan|Both`, and `-RunId` parameters. `-Soak` is valid only for the Candidate stage; `-Finalize` is valid only for the final Day 40 sign-off operation.
...
8. Emit `candidate-draft.json` with evidence chain, stage results, hashes, paths, thresholds, known limitations, and separate local/external dispositions. The `-Soak` operation emits the Day 39 result. Day 40 invokes `-Finalize`, consumes both records, re-runs the final four-lane matrix, and writes `candidate.json` exactly once as the immutable final artifact.
