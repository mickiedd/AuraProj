...
- **New output:** `Saved/Reports/PlayableCandidate/<Revision>/<RunId>/day-22-baseline.json`; do not overwrite historical Day 20 evidence.

### Detailed steps

1. Capture revision, engine/platform, binary paths, run ID, timeout policy, and clean/dirty state before launching any process.
2. Build the required Editor, Game, and Server Development/Shipping targets from a fresh invocation; record stdout/stderr, exit code, and produced binary timestamp.
...
5. Execute existing persistence and multiplayer listen/dedicated probes with new per-run log paths, ports, client identities, and `WorldPersistenceId` values.
...
- **Required artifacts:** `Saved/Reports/PlayableCandidate/<Revision>/<RunId>/day-22-baseline.json`, command/exit-code table, per-process cleanup record, staged XML manifest report, and a list of reproduced failures.
