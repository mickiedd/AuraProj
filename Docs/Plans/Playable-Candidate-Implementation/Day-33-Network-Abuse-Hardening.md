...
Every client-originated action is documented as `endpoint → owner check → session/nonce → target/state/range → rate/replay → authoritative commit → result code`. Requests contain only stable IDs and client-observed intent; prices, quantities, damage, ammo result, role, stock, save, and success values are server-resolved. A rejected request is side-effect free and produces a bounded typed result.

### Detailed steps

1. Enumerate all RPCs, reliable/unreliable commands, Blueprint-callable functions, console commands, WebUI bridge messages, test hooks, grant helpers, and subsystem entry points introduced or touched by Days 25–32.
...
8. Scan packaged Shipping outputs and archive the ownership table before Day 34.
