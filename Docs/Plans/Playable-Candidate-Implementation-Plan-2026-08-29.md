...
- **Diagnostics privacy:** logs use a run-scoped HMAC representation of player identity plus provider type; the HMAC key is supplied to cooperating processes but never enters logs or candidate artifacts, so identities correlate within a run without being dictionary-hashable or linkable across runs. Raw external identifiers, credentials, tokens, and personal data never enter the candidate artifact.
- **GSM control plane:** loopback is the default bind. A non-loopback bind is refused unless request authentication, dedicated-server readiness authentication, and an explicit IP/CIDR client allowlist are configured. `request_server` and `server_ready` are authenticated before any launch or readiness mutation; each managed launch also carries a one-time readiness nonce, readiness is accepted only for the matching configured level and port, `AURA_GSM_ADDRESS` is the explicit connect endpoint when the bind host differs, and clients receive the manager-configured public endpoint.
...
- `Saved/Reports/PlayableCandidate/<SourceRevision>/<RunId>/candidate-draft.json` — Day 38 provisional machine artifact. Day 40 finalization consumes explicitly named draft/soak input paths with matching `RunId`, scope hash, source revision, and package/content hashes; it never selects a latest artifact. `candidate.json` is written once by Day 40 as the immutable final artifact. Markdown/SVG explain them but do not replace either machine record.
