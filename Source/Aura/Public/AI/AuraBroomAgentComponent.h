 ...
 *
 * Aura-specific BehaviorU agent for the broom vehicle (AAuraBroomVehicle). This
 ...
 * BehaviorU-agnostic and this component thin.
 *
 * Bound methods:
 ...
 *
 * The methods execute on the game thread (Phase 1 of the BehaviorU world
 * subsystem tick), so they may safely touch the owning broom actor.
 */
UCLASS(ClassGroup = (AI), meta = (BlueprintSpawnableComponent), DisplayName = "Aura Broom BehaviorU Agent")
class AURA_API UAuraBroomAgentComponent : public UBehaviorUAgentComponent
{
	GENERATED_BODY()
...
protected:
	...
	EBehaviorUStatus Method_FindPlayer();

	/** Delegate the follow thrust/yaw decision to the vehicle and apply it. */
	EBehaviorUStatus Method_FollowPlayer();
