...
float AAuraBroomVehicle::GetTimeSinceLastDismount() const
{
...
FVector AAuraBroomVehicle::ComputeBtFollowThrust(const FVector& PlayerLocation, float& OutTargetYaw, bool& bOutHasTargetYaw) const
{
	OutTargetYaw = 0.f;
	...
	if (bStopFollowWhenMounted && IsValid(MountedCharacter))
	{
	...
	const FVector BroomLocation = GetActorLocation();
	...
	const float TimeSinceDismount = GetTimeSinceLastDismount();
	const bool bRecentDismount = IsValid(LastDismountedCharacter)
