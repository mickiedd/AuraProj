...
{
	TestNull(TEXT("Retired FireGun projectile definition stays absent"),
		FAuraGameplayConfig::FindProjectile(TEXT("fireGunBullet")));
	TestFalse(TEXT("Retired FireGun ability definition stays absent"),
		FPaths::FileExists(FPaths::ProjectContentDir() / TEXT("AbilityDefinitions/FireGun.xml")));
	return !HasAnyErrors();
}
...
bool FAuraProjectileDefinitionsTest::RunTest(const FString& Parameters)
{
	...
	TestNull(TEXT("Retired FireGun projectile is absent"), FAuraGameplayConfig::FindProjectile(TEXT("fireGunBullet")));
...
bool FAuraPlayerSkillAuthoritySafetyTest::RunTest(const FString& Parameters)
{
	...
	for (const FString& AbilityFile : AbilityFiles)
	{
		...
		// Gameplay-expansion boss attacks use a separate schema in this shared directory.
		if (Xml.Contains(TEXT("<bossAttack")))
		{
			continue;
		}
